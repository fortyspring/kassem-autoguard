...
function sod_render_world_monitor_dashboard($atts = []): string {
    ...
    <div class="sod-wm" id="<?php echo esc_attr($uid); ?>" dir="rtl">
        <style>
            ...
            #<?php echo esc_html($uid); ?> .sod-wm-head-card{padding:14px 16px;border-radius:18px;background:linear-gradient(180deg,rgba(14,22,34,.98),rgba(9,15,23,.98));border:1px solid var(--line);display:grid;gap:8px;align-content:center;box-shadow:inset 0 1px 0 rgba(255,255,255,.03);direction:rtl;text-align:right}
            ...
            #<?php echo esc_html($uid); ?> .sod-wm-sub{margin-top:6px;color:var(--muted);font-size:13px}
            ...
            #<?php echo esc_html($uid); ?> .sod-wm-nav{display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin-bottom:14px;padding:10px 12px;border-radius:18px;background:linear-gradient(180deg,rgba(13,20,31,.96),rgba(8,13,19,.96));border:1px solid var(--line);direction:ltr}
            ...
            #<?php echo esc_html($uid); ?> .sod-wm-kpi>div:nth-child(2){direction:rtl;text-align:right}
            #<?php echo esc_html($uid); ?> .sod-wm-kpi-top{display:flex;align-items:center;justify-content:space-between;gap:10px;color:#cfe0f4;font-size:13px;direction:rtl}
            ...
            #<?php echo esc_html($uid); ?> .sod-wm-card-head{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:14px;direction:rtl;text-align:right}
            ...
            #<?php echo esc_html($uid); ?> .sod-wm-legend{display:flex;align-items:center;justify-content:space-between;gap:10px;font-size:13px}
            ...
            #<?php echo esc_html($uid); ?> .sod-wm-video-tabs{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:12px;direction:ltr}
            ...
            #<?php echo esc_html($uid); ?> .sod-wm-live-status-item{padding:14px 12px;border-radius:14px;background:linear-gradient(180deg,#101926,#0c141e);border:1px solid rgba(255,255,255,.06);display:flex;align-items:center;justify-content:space-between;gap:10px;direction:ltr}
            ...
            #<?php echo esc_html($uid); ?> .sod-wm-top-region{display:grid;grid-template-columns:110px minmax(0,1fr) 48px;gap:10px;align-items:center}
            ...
            #<?php echo esc_html($uid); ?> .sod-wm-table th,#<?php echo esc_html($uid); ?> .sod-wm-table td{padding:12px 10px;border-bottom:1px solid rgba(255,255,255,.06);text-align:right;font-size:13px;vertical-align:top}
            ...
            @keyframes sod-wm-marquee{0%{transform:translateX(0)}100%{transform:translateX(-50%)}}
            ...
            @media (max-width:760px){
                ...
                #<?php echo esc_html($uid); ?> .sod-wm-strip{grid-template-columns:1fr}
        ...
        (() => {
            ...
            function renderVideoWall(streams, urgentItems) {
                ...
                const ordered = channels.slice(state.activeVideo).concat(channels.slice(0, state.activeVideo)).slice(0, 4);
                setHtml('video_grid', ordered.map((stream, index) => {
                    const headline = urgentItems[index] || urgentItems[0] || {};
                    const top = `
                    ...
                    if (stream.embed_url) {
                        return `<div class="sod-wm-video-tile">${top}<iframe src="${esc(stream.embed_url)}" title="${esc(stream.name || 'Live Stream')}" loading="lazy" referrerpolicy="strict-origin-when-cross-origin" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture; fullscreen" allowfullscreen></iframe>${time}</div>`;
                    }
            ...
            function renderTopRegions(regions) {
                ...
                setHtml('top_regions', regions.slice(0, 5).map((item, index) => {
                    ...
                    return `<div class="sod-wm-top-region">
                        ...
                        <span style="text-align:left;font-weight:800">${Math.round((value / maxValue) * 32) || 1}%</span>
                    </div>`;
                }).join(''));
            }
